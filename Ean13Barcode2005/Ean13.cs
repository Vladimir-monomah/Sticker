...
namespace Ean13Barcode2005
{
	public class Ean13
	{
        ...
        // Это номинальный размер, рекомендованный EAN.
		...
		public void DrawEan13Barcode( System.Drawing.Graphics g, System.Drawing.Point pt )
		{
            ...
            //	Штрих-код EAN13 должен иметь ширину 113 модулей.
            float lineWidth = width / 113f;

            // Сохраните GraphicsState.
            System.Drawing.Drawing2D.GraphicsState gs = g.Save( );

            // Установите для PageUnit значение Inch, потому что все наши измерения в дюймах.
            g.PageUnit = System.Drawing.GraphicsUnit.Millimeter;

            // Установите PageScale на 1, так что миллиметр будет соответствовать истинному миллиметру.
            g.PageScale = 1;
            ...
            for ( int i = 0; i < strbEAN13.Length; i++ )
			{
				if( sTempUPC.Substring( i, 1 ) == "1" )
				{
                    ...
                    // Оставьте место для номера UPC под штрих-кодом.
                    if ( ( i > 12 && i < 55 ) || ( i > 57 && i < 101 ) )
                        // Нарисуйте пространство для числа
                        g.FillRectangle( brush, xPosition, yStart, lineWidth, height - fTextHeight );
					else
                        // Проведите полную линию.
		...
		{
            // 0 Нечетный Нечетный Нечетный Нечетный Нечетный
		...
		{
            // 1 Нечетный Нечетный Четный Нечетный Четный Четный
            System.Text.StringBuilder sReturn = new StringBuilder( );
            // Две приведенные ниже строки можно заменить на следующие:
		...
		{
            // 2 Нечетный Нечетный Четный Нечетный Четный
            System.Text.StringBuilder sReturn = new StringBuilder( );
			sReturn.Append( ConvertToDigitPatterns( sLeft.Substring( 0, 1 ), this._aOddLeft ) );
			sReturn.Append( ConvertToDigitPatterns( sLeft.Substring( 1, 1 ), this._aOddLeft ) );
			sReturn.Append( ConvertToDigitPatterns( sLeft.Substring( 2, 1 ), this._aEvenLeft ) );
		...
		{
            // 3 Нечетный Нечетный Четный Четный Нечетный
            System.Text.StringBuilder sReturn = new StringBuilder( );
			sReturn.Append( ConvertToDigitPatterns( sLeft.Substring( 0, 1 ), this._aOddLeft ) );
			sReturn.Append( ConvertToDigitPatterns( sLeft.Substring( 1, 1 ), this._aOddLeft ) );
			sReturn.Append( ConvertToDigitPatterns( sLeft.Substring( 2, 1 ), this._aEvenLeft ) );
		...
		{
            // 4 Нечетный Четный Нечетный Четный Четный
            System.Text.StringBuilder sReturn = new StringBuilder( );
			sReturn.Append( ConvertToDigitPatterns( sLeft.Substring( 0, 1 ), this._aOddLeft ) );
		...
		{
            // 6 Нечетный Четный Четный Нечетный Нечетный Четный
            System.Text.StringBuilder sReturn = new StringBuilder( );
			sReturn.Append( ConvertToDigitPatterns( sLeft.Substring( 0, 1 ), this._aOddLeft ) );
			sReturn.Append( ConvertToDigitPatterns( sLeft.Substring( 1, 1 ), this._aEvenLeft ) );
			sReturn.Append( ConvertToDigitPatterns( sLeft.Substring( 2, 1 ), this._aEvenLeft ) );
		...
		{
            // 7 Нечетный Четный Нечетный Четный Нечетный Четный
            System.Text.StringBuilder sReturn = new StringBuilder( );
			sReturn.Append( ConvertToDigitPatterns( sLeft.Substring( 0, 1 ), this._aOddLeft ) );
			sReturn.Append( ConvertToDigitPatterns( sLeft.Substring( 1, 1 ), this._aEvenLeft ) );
			sReturn.Append( ConvertToDigitPatterns( sLeft.Substring( 2, 1 ), this._aOddLeft ) );
		...
		{
            // 8 Нечетный Четный Нечетный Четный Нечетный
            System.Text.StringBuilder sReturn = new StringBuilder( );
			sReturn.Append( ConvertToDigitPatterns( sLeft.Substring( 0, 1 ), this._aOddLeft ) );
			sReturn.Append( ConvertToDigitPatterns( sLeft.Substring( 1, 1 ), this._aEvenLeft ) );
			sReturn.Append( ConvertToDigitPatterns( sLeft.Substring( 2, 1 ), this._aOddLeft ) );
		...
		{
            // 9 Нечетный Четный Четный Нечетный Четный Нечетный
            System.Text.StringBuilder sReturn = new StringBuilder( );
			sReturn.Append( ConvertToDigitPatterns( sLeft.Substring( 0, 1 ), this._aOddLeft ) );
			sReturn.Append( ConvertToDigitPatterns( sLeft.Substring( 1, 1 ), this._aEvenLeft ) );
			sReturn.Append( ConvertToDigitPatterns( sLeft.Substring( 2, 1 ), this._aEvenLeft ) );
		...
		public void CalculateChecksumDigit( )
		{
            ...
            // Вычислите здесь контрольную сумму.
		...
		public float Scale
		{
			...
			set
			{
				if( value < this._fMinimumAllowableScale || value > this._fMaximumAllowableScale )
					throw new Exception("Масштабировать значение вне допустимого диапазона. Значение должно быть между " + 
						this._fMinimumAllowableScale.ToString( ) + " и " + 
		...
		public string ChecksumDigit
		{
			...
			set
			{
				...
				if( iValue < 0 || iValue > 9 )
					throw new Exception("Контрольная цифра должна быть от 0 до 9.");
